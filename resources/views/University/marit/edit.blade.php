                ...
                <div class="col-md-6 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form class="forms-sample" id="submit_form" action="{{ route('university.marit.update',$marit_round->id) }}"
                                method="post" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <input type="hidden" name="id" value="{{$marit_round->id}}">
                                <div class="form-group">
                                    <label for="basicInput">Round No</label>
                                    <input type="text" class="form-control mb-2" name="round_no"
                                        value="{{ $marit_round->round_no }}" placeholder="Name">
                                    @error('round_no')
                                    ...
                                    <select class="form-control" id="course_id" name="course_id">
                                        ...
                                        @foreach ($course as $value)
                                            <option value="{{ $value->id }}"
                                                {{ old('subject', isset($marit_round->course_id) ? $marit_round->course_id : '') == $value->id ? 'selected' : '' }}>
                                                {{ $value->name }}</option>
                                        @endforeach
                                        @error('course_id')
                                        <span class="text-danger" role="alert">
                                ...
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type='tax' class="form-control datepicker" name="start_date" value="{{$marit_round->start_date}}" id="start_date"
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type='tax' name="end_date" class="form-control datepicker" value="{{$marit_round->end_date}}" id='end_date'
                                            placeholder='Select Date' style='width: 180px;' autocomplete="off">
                                            @error('end_date')
                                            <span class="text-danger" role="alert">
                                                <strong>{{ $message }}</strong>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type='tax' class="form-control datepicker" value="{{ $marit_round->merit_result_declare_date }}"
            ...
            swal({
                ...
                if (result) {
                    $.ajax({
                        ...
                        type: 'POST',
                        url: "{{ route('university.marit.update',$marit_round->id) }}",
                        data: formData,
                        dataType: 'JSON',
                        contentType: false,
                        processData: false,
